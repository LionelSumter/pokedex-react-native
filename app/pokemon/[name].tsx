...
export default function PokemonDetailScreen() {
  ...
  // Dynamic heights per scene → outer ScrollView scrolls
  ...
  /** ---------- Scenes (hooks before any early returns) ---------- */

  const AboutScene = useCallback(
    () => {
      if (!pokemon) return <View onLayout={handleLayoutFor('about')} />;
      return (
    ...
    [pokemon, abilities, typesLine, handleLayoutFor]
  );

  const StatsScene = useCallback(
    () => {
      if (!pokemon) return <View onLayout={handleLayoutFor('stats')} />;
      return (
  ...
  const EvolutionScene = useCallback(
    ...
    [evoSteps, isEvoLoading, handleLayoutFor]
  );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#E9F2F8' }}>
      ...
      >
        ...
        {/* ==== Tabs (custom header; default TabBar off) ==== */}
