...
function createStyles(
  ...
  return StyleSheet.create({
    ...
    aboutVal: {
      ...
      lineHeight: 20,
      color:
        scheme === "dark" ? tokens.color.text.muted : "rgba(14, 9, 64, 0.60)",
      textTransform: "capitalize",
    },
    ...
    evoWell: {
      ...
      height: "100%",
      backgroundColor:
        scheme === "dark" ? "rgba(233, 230, 255, 0.06)" : DETAIL.evoWellBg,
      alignItems: "center",
      justifyContent: "center",
    },
    evoSprite: {
    ...
    <View
      ...
      onLayout={(e) => {
        ...
        setSceneHeights((prev) =>
          prev.about === h ? prev : { ...prev, about: h },
        );
      }}
    >
      <AboutRow
      ...
      <AboutRow
        label={i18n.t("details.base")}
        value={String(pokemon.base_experience ?? "-")}
      />
      <AboutRow
        label={i18n.t("details.weight")}
        value={`${(pokemon.weight ?? 0) / 10} kg`}
      />
      <AboutRow
        label={i18n.t("details.height")}
        value={`${(pokemon.height ?? 0) / 10} m`}
      />
      <AboutRow
        label={i18n.t("details.types")}
        value={types.map(titleCase).join(", ") || "-"}
      />
      <AboutRow
  ...
  const renderStats = () => (
    <View
      ...
      onLayout={(e) => {
        const h = e.nativeEvent.layout.height;
        setSceneHeights((prev) =>
          prev.stats === h ? prev : { ...prev, stats: h },
        );
      }}
    >
    ...
    >
      {evoLoading ? (
        ...
        <View style={[s.centerSmall, { paddingTop: contentTopPad }]}>
          <Text style={s.errorTitle}>{i18n.t("common.error")}</Text>
          <Text style={s.muted}>
            {evoError instanceof Error
              ? evoError.message
              : i18n.t("details.failedToLoadEvolution")}
          </Text>
        </View>
      ) : (
  ...
  return (
    <SafeAreaView style={s.safe} edges={["left", "right"]}>
      <View style={s.container}>
        <View
          style={[s.headerRow, { height: headerH, paddingTop: insets.top }]}
        >
          <BlurView
            intensity={50}
        ...
        >
          <View
            style={[s.heroBg, { height: sheetTopY }]}
            pointerEvents="none"
          />
              ...
              <View style={s.chipsRow}>
                {(types ?? []).map((t) => (
                  <View key={t} style={s.chip}>
                    <View
                      style={[s.dot, { backgroundColor: getTypeColor(t) }]}
                    />
          ...
          >
            <View style={{ height: tabViewHeight }}>
              <TabView
                ...
                swipeEnabled
                renderTabBar={(props) => (
                  <View style={s.tabsWrap} pointerEvents="box-none">
